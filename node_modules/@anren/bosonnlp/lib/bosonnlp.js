...
BosonNLP.prototype.suggest = function (data, callback) {
	...
	var datas = parseDatas(data);
	// console.log(datas.slice(1,-1));
	sendPost(postOptions,datas.slice(1,-1), callback);
};

// add summary by nhj
BosonNLP.prototype.summary = function (data, percentage, not_exceed, callback) {
	//percentage <= 1，则认为是摘要字数与原文总字数的百分比；
	//percentage > 1， 则认为是最终摘要的具体字数
	//not_exceed: 是否为严格字数限制
	//false:不开启，摘要字数大于等于字数限制以确保句子完整
	//true:开启，摘要字数小于等于字数限制
	postOptions.path = "/summary/analysis";
	//var datas = parseDatas(data);
	sendPost(postOptions, JSON.stringify({content:data,percentage:percentage,not_exceed:not_exceed}), callback);
};
